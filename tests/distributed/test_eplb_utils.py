...
@pytest.mark.parametrize("num_physical", [16, 512, 576])
def test_record_expert_load(num_physical):
    """Match logical counts and clear only active counters when reusing history rows."""
    torch.manual_seed(0)
    mapping = torch.randint(-1, 17, (3, num_physical + 7), device="cuda")
    counters = torch.full(mapping.shape, -9, dtype=torch.int32, device="cuda")
    window = torch.full((2, 3, 18), -9, dtype=torch.int32, device="cuda")
    expected = window.clone()
    active_mapping = mapping[:, :num_physical]
    active_counters = counters[:, :num_physical]
    for step in (0, 1, 0):
        active_mapping.copy_(active_mapping.flip(-1))
        active_counters.random_(0, 100)
        expected[step].zero_().scatter_add_(
            -1, active_mapping.masked_fill(active_mapping < 0, 17), active_counters
        )
        record_expert_load(active_counters, active_mapping, window, step)
        torch.testing.assert_close(window, expected)
        assert not active_counters.any().item()
        assert (counters[:, num_physical:] == -9).all().item()
