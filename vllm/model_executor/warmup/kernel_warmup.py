...
def kernel_warmup(worker: "Worker", *, process_local_only: bool = False):
    ...
    if not worker.use_v2_model_runner:
        ...
        if not worker.model_runner.is_pooling_model:
            with record_commit_stage(
                "kernel_warmup.v1_block_table", synchronize_gpu=True
            ):
        ...
        if zeroer is not None:
            with record_commit_stage(
                "kernel_warmup.kv_block_zeroer", synchronize_gpu=True
            ):
    ...
    if worker.vllm_config.kernel_config.enable_jit_warmup:
        with record_commit_stage("kernel_warmup.kimi_k3", synchronize_gpu=True):
