...
class EplbModelState:
    ...
    """Logical counters followed by optional physical counters for load logging.

    Physical counters reserve maximum EP capacity to keep graph pointers stable.
    """
...
class EplbState:
    ...
    def add_model(
        ...
        num_load_counters = model.num_logical_experts
        if self.parallel_config.eplb_config.log_balancedness:
            num_load_counters += physical_expert_capacity
        expert_load_pass_buffer = torch.zeros(
            (model.num_moe_layers, num_load_counters),
            dtype=torch.int32,
            device=self.device,
        )
        expert_load_pass = expert_load_pass_buffer[:, : model.num_logical_experts]
    ...
    def step(
        ...
        if is_dummy:
            ...
            for eplb_model_state in self.model_states.values():
                eplb_model_state.expert_load_pass_buffer.zero_()
    ...
    def reconfigure_physical_expert_slots(
        ...
        physical_load = model_state.expert_load_pass_buffer[
            :, model_state.model.num_logical_experts :
        ]
        physical_load[:, expert_slots].zero_()
...
class EplbLayerState:
    ...
    """Logical counters followed by optional maximum-capacity physical counters."""
