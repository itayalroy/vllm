...
from .eplb_utils import CpuGpuEvent, record_expert_load
...
class EplbModelState:
    ...
    physical slots are reassigned. The final column lets the PyTorch fallback
    discard inactive-slot counts and is excluded from rebalancing.
...
class EplbState:
    ...
    def add_model(
        ...
        # Compile recording before serving; both buffers are still empty.
        record_expert_load(
            ...
            for eplb_model_state in self.model_states.values():
                if should_record:
                    record_expert_load(
                        ...
                        eplb_model_state.physical_to_logical_map_buffer,
