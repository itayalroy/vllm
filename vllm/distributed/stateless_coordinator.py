...
def _allocate_group_ports(
    ...
    sockets still open so the caller can hand them off via
    ``master_listen_fd``.
    """
...
class StatelessGroupCoordinator(GroupCoordinator):
    ...
    def __init__(
        ...
        assert group_ports is not None or coord_store is not None, (
            "either group_ports or coord_store must be provided"
        )
            ...
            if self.rank in ranks:
                ...
                if coord_store is not None:
                    ...
                    if self.rank_in_group == 0:
                        ...
                        self._held_sockets.extend(socks)
                    else:
                        ports = _fetch_group_ports(key, coord_store)
                        socks = []
                else:
                    assert group_ports is not None
                    ports = group_ports[idx]
                    socks = []
                device_port, cpu_port, tcp_store_port = ports
                device_fd = socks[0].fileno() if socks else None
                cpu_fd = socks[1].fileno() if socks else None
                tcp_store_sock = socks[2] if socks else None
