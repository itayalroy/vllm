...
class ScaleUpExistingEngineState(enum.IntEnum):
    ...
    TRANSFER_EXPERT_MAPPING = 1
    TRANSFER_WEIGHTS = 2
    SYNC_KV_CACHE_MEMORY_SIZE = 3
    WAIT_NEW_WORKERS_PREPARED = 4
    COMMIT_SCALE_UP = 5
    COMPLETE = 6
...
class ElasticEPScalingState:
    def __init__(
        ...
        self.prepare_mode = worker_type == "existing"
    ...
    def _execute_async(self, execute_method: str, *args) -> bool:
        if self._prepare_async_done_keys is None:
    ...
    def _progress_existing_engine(self) -> bool:
        ...
        if state == ScaleUpExistingEngineState.CREATE_STANDBY_GROUPS:
            ...
            if not self._sync_kv_cache_memory_size():
                return False
            self.state = ScaleUpExistingEngineState.WAIT_NEW_WORKERS_PREPARED
            return True

        elif state == ScaleUpExistingEngineState.WAIT_NEW_WORKERS_PREPARED:
            if not self._sync_new_workers_prepared(async_op=True):
                return False
    ...
    def _progress_new_engine(self) -> bool:
        ...
        if state == ScaleUpNewEngineState.PRE_KV_INIT:
            ...
            self._sync_new_workers_prepared(async_op=False)
    ...
    def _ensure_new_dp_group(self) -> bool:
        ...
        future = self._prepare_dp_group_runner.start(self._init_new_dp_group)
        if not future.done():
            return False

        self.new_dp_group, self.new_dp_store = self._prepare_dp_group_runner.clear()
        return True
    ...
    def _transfer_expert_mapping(self) -> bool:
        assert self.old_dp_group is not None
        if not self._execute_async("broadcast_expert_mapping"):
            return False
        if self.old_dp_group.rank() == 0:
    ...
    def _sync_kv_cache_memory_size(self) -> bool:
        ...
        if self._new_dp_sync is None:
            ...
            work = self._new_dp_all_reduce(tensor, async_op=True)
            self._new_dp_sync = (tensor, work)
            return False

        if not self._new_dp_all_reduce_done(self._new_dp_sync):
            return False
    ...
    def _sync_new_workers_prepared(self, async_op: bool) -> bool:
        assert self.new_dp_group is not None

        if self._new_dp_sync is None:
            tensor = torch.tensor([1], dtype=torch.int32, device="cpu")
            work = self._new_dp_all_reduce(tensor, async_op)
            if not async_op:
                return True
            self._new_dp_sync = (tensor, work)
            return False
    ...
    @staticmethod
    def _new_dp_all_reduce_done(sync: tuple[object, Any]) -> bool:
        _, work = sync
    ...
    def _new_dp_all_reduce(self, tensor, async_op: bool):
        assert self.new_dp_group is not None
