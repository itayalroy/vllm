...
class ElasticEPScalingExecutor:
    ...
    def warm_and_capture(self) -> None:
        ...
        reuse_cuda_graphs = self._can_reuse_cuda_graphs()
        with (
            runner.skip_dp_coordination() if reuse_cuda_graphs else nullcontext(),
            all2all_manager.mask_remote_ranks() if reuse_cuda_graphs else nullcontext(),
            self._disable_flashinfer_autotune() if reuse_cuda_graphs else nullcontext(),
        ):
