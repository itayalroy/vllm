...
class NixlEPAll2AllManager(All2AllManagerBase):
    ...
    def _init_buffer(
        ...
        buffer.connect_ranks(ranks_to_connect, activate=activate)
        NixlEPAll2AllManager._buffer = _NixlEPBufferState(
            buffer=buffer,
            connected_ep_size=self.world_size,
            active_ep_size=self.world_size if activate else 0,
            max_num_tokens_per_dp_rank=max_num_tokens_per_dp_rank,
            token_hidden_size=token_hidden_size,
            num_experts_per_rank=num_experts_per_rank,
        )

    def _matches_buffer_shape(
        self,
        state: _NixlEPBufferState,
        max_num_tokens_per_dp_rank: int,
        token_hidden_size: int,
        num_experts_per_rank: int,
    ) -> bool:
        return (
            state.max_num_tokens_per_dp_rank == max_num_tokens_per_dp_rank
            and state.token_hidden_size == token_hidden_size
            and state.num_experts_per_rank == num_experts_per_rank
        )
    ...
    def _update_memory_buffers(
        ...
        state.max_num_tokens_per_dp_rank = max_num_tokens_per_dp_rank
        state.token_hidden_size = token_hidden_size
        state.num_experts_per_rank = num_experts_per_rank

    def _connect_to_ep_size(self, ep_size: int, *, activate: bool) -> None:
        assert NixlEPAll2AllManager._buffer is not None
        state = NixlEPAll2AllManager._buffer
    ...
    def _disconnect_to_ep_size(self, ep_size: int) -> None:
        ...
        state.buffer.set_tcp_store_group(self.tcp_store_group.store)
        for rank in range(ep_size, state.active_ep_size):
    ...
    def _update_buffer(
        ...
        *,
        ...
        assert NixlEPAll2AllManager._buffer is not None
        state = NixlEPAll2AllManager._buffer
        new_ep_size = self.world_size
        if new_ep_size < state.connected_ep_size:
            self._disconnect_to_ep_size(new_ep_size)
        ...
        if new_ep_size > state.connected_ep_size:
            self._connect_to_ep_size(new_ep_size, activate=activate)
        elif activate and new_ep_size > state.active_ep_size:
            self.activate_connected_ranks()
    ...
    def get_handle(self, kwargs):
        with NixlEPAll2AllManager._lock:
            ...
            if state is not None:
                has_expected_shape = self._matches_buffer_shape(
                    state,
                ...
                is_ready = (
                    ...
                    if activate
                    else state.connected_ep_size == self.world_size
                )
                if has_expected_shape and is_ready:
                    return state.buffer
